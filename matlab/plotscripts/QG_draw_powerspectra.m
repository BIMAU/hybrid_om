...
f = plot_spectra(qg_c, p, plot_data(1:4), ...
                 names(1:4), colors(1:4), spec_opts, styles, msizes);
...
f = plot_spectra(qg_c, p, plot_data, names, colors, spec_opts, styles, msizes);
Utils.exportfig([exportdir, 'results_powerspec_rom.eps'], fs, dims, invert)
%-----------------------------------------------------------------------------

function [f] = plot_spectra(qg, p, plot_data, names, colors, opts, styles, msizes)
    f = [];
    opts.T = size(plot_data{1},2);
    [Pm, Pv] = Utils.get_qg_mean_spectrum(qg, plot_data{1}, opts);
    opts.norm_factor = max(Pm);
